...
def arg_parse():
    ...
    parser.add_argument('-model', type = str, default = 'regression', help = 'options: moneyline vs total vs binary_regression')
    ...
    parser.add_argument('-pred_fname', type = str, default = 'reg_10_20_player_model_uniqid_cos_sch_512_256_d_0.3_b_512_l1', help = 'model prediction for binary output')
...
if __name__ == '__main__':
    ...
    if not arg.load_processed_data:
        ...
        # TODO : Comment below when testing adding player features
        ...
        model_prediction = pathlib.Path.cwd().parent / 'output' / arg.pred_fname + '_pred_output.feather'
        X_test_df_tmp = pathlib.Path.cwd().parent / 'output' / arg.pred_fname + '_source_data.feather'
        pred_cols = model_prediction.columns[model_prediction.columns.str.contains('y_test_pred_seed')].tolist()
        tmp_cols = pred_cols + ['ytest_pred_mean', 'Date', 'Home', 'Away']
        df_all = pd.concat([model_prediction, X_test_df_tmp], axis=1, ignore_index=False)
        df_filt = df_all[tmp_cols]
        df = df.merge(df_filt, left_on = ['Date', 'Home', 'Away'], right_on = ['Date', 'Home', 'Away'])
    ...
    features = features + all_features + df.columns[df.columns.str.contains('_game_played')].tolist() + count_cols + pred_cols
